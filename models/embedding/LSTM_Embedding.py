...
def run_embedding(batch_size, epochs, repeats, model_name):
    ...
    def LSTM_embedding(input_shape: tuple, target_shape: tuple) -> keras.Model:
        inputs = []
        embed_layers = []
        ...
        x = layers.LSTM(return_sequences=True, units=64, activation="relu")(x)
        x = layers.LSTM(return_sequences=True, units=64, activation="relu")(x)
        x = layers.LSTM(return_sequences=True, units=64, activation="relu")(x)
        ...
        x = layers.Flatten()(x)
        x = layers.Dropout(0.2)(x)
        x = layers.Dense(target_shape[0], activation='sigmoid')(x)

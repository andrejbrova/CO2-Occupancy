...
def load_dataset(
        dataset='Australia',  # 'uci', 'Australia', 'Denmark', 'Italy'
    ...
    if dataset == 'uci':
        ...
        if dataset=='Denmark':
            testData = data.loc['6/21/2018  12:00:00 AM':'9/14/2018  11:59:00 PM', :]
            trainData = data.drop(testData.index)
        elif dataset=='Italy':
            testData = data.loc['6/21/2016  12:00:00 AM':'9/21/2016  11:59:00 PM', :]
            trainData = data.drop(testData.index)
        ...
        y_train = pd.DataFrame(trainData.loc[:, 'Occupancy'])
        y_test = pd.DataFrame(testData.loc[:, 'Occupancy'])

        if normalize:
            X_train_scaleable = X_train.select_dtypes(exclude='category')
            x_train_scaler = dm.processing.Normalizer().fit(X_train_scaleable)
            X_train_scaleable = x_train_scaler.transform(X_train_scaleable)
            X_train[X_train_scaleable.columns] = X_train_scaleable
            X_test_scaleable = X_test.select_dtypes(exclude='category')
            x_test_scaler = dm.processing.Normalizer().fit(X_test_scaleable)
            X_test_scaleable = x_test_scaler.transform(X_test_scaleable)
            X_test[X_test_scaleable.columns] = X_test_scaleable
